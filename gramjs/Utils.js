...
function _raiseCastFail(entity, target) {
  throw new Error(
    `Cannot cast ${entity.constructor.name} to any kind of ${target}`
  );
}
...
function getInputPeer(entity, allowSelf = true, checkHash = true) {
  ...
  if (
    entity instanceof types.Chat ||
    entity instanceof types.ChatEmpty ||
...
function getInputChannel(entity) {
  ...
  if (
    entity instanceof types.Channel ||
    entity instanceof types.ChannelForbidden
  ) {
...
function getInputUser(entity) {
  ...
  if (
    entity instanceof types.UserEmpty ||
    entity instanceof types.InputPeerEmpty
  ) {
...
function getInputPhoto(photo) {
  ...
  if (
    photo instanceof types.photos.Photo ||
  ...
  if (
    photo instanceof types.UserEmpty ||
...
function getInputMedia(
  ...
  if (media instanceof types.Document || media instanceof types.DocumentEmpty) {
    return new types.InputMediaDocument({
      id: getInputDocument(media),
    });
  }

  if (media instanceof types.InputFile || media instanceof types.InputFileBig) {
  ...
  if (
    media instanceof types.MessageMediaEmpty ||
...
function getPeer(peer) {
  try {
    ...
    if (peer.SUBCLASS_OF_ID === 0x2d45687) {
      ...
      peer instanceof types.contacts.ResolvedPeer ||
      peer instanceof types.InputNotifyPeer ||
      peer instanceof types.TopPeer ||
      peer instanceof types.Dialog ||
      peer instanceof types.DialogPeer
    ) {
      return peer.peer;
...
function _rleDecode(data) {
  return data.replace(/(\d+)([A-z\s])/g, (_, runLength, char) =>
...
function resolveBotFileId(fileId) {
  ...
  if (
    (version === 2 && data.size === 24) ||
    (version === 4 && data.size === 25)
  ) {
    // eslint-disable-next-line one-var
    let fileType, dcId, mediaId, accessHash;
    if (version === 2) {
      [fileType, dcId, mediaId, accessHash] = struct.unpack(
        "<iiqq",
        Buffer.from(data)
      );
    } else {
    ...
    switch (fileType) {
      ...
      case 8:
        attributes.push(
          new types.DocumentAttributeSticker({
            alt: "",
    ...
    (version === 2 && data.size === 44) ||
    (version === 4 && data.size === 49)
  ) {
    // eslint-disable-next-line one-var
    let dcId, mediaId, accessHash, volumeId, localId;
    if (version === 2) {
      [, dcId, mediaId, accessHash, volumeId, , localId] = struct.unpack(
        "<iiqqqqi",
        Buffer.from(data)
      );
    } else {
