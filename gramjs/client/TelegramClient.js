...
} = require("../network/connection/TCPObfuscated");
...
class TelegramClient {
  ...
  constructor(session, apiId, apiHash, opts = TelegramClient.DEFAULT_OPTIONS) {
    ...
    if (
      ...
      session.serverAddress.includes(":") !== this._useIPV6
    ) {
      session.setDC(
        DEFAULT_DC_ID,
  ...
  async _getDC(dcId, cdn = false) {
    ...
    if (cdn && !this._cdnConfig) {
      this._cdnConfig = await this.invoke(
        new functions.help.GetCdnConfigRequest()
      );
    ...
    for (attempt = 0; attempt < this._requestRetries; attempt++) {
      try {
        ...
        if (
          ...
          e instanceof errors.RpcCallFailError ||
          ...
          this._floodWaitedRequests[request.CONSTRUCTOR_ID] =
            new Date().getTime() / 1000 + e.seconds;
  ...
  async getMe() {
    const me = (
      await this.invoke(
        new functions.users.GetUsersRequest({ id: [new types.InputUserSelf()] })
      )
    )[0];
    return me;
  }
  ...
  async start(
    ...
    while (attempts < args.maxAttempts) {
      try {
        ...
        if (e instanceof errors.SessionPasswordNeededError) {
          ...
          e instanceof errors.PhoneCodeEmptyError ||
    ...
    if (attempts >= args.maxAttempts) {
      throw new Error(
        `${args.maxAttempts} consecutive sign-in attempts failed. Aborting`
      );
    }
    if (twoStepDetected) {
      if (!args.password) {
  ...
  async signIn(
    ...
    if (args.phone && !args.code && !args.password) {
      ...
      console.log(this._phoneCodeHash[this._phone]);
      const [phone, phoneCodeHash] = this._parsePhoneAndHash(
        args.phone,
        args.phoneCodeHash
      );
      ...
      result = await this.invoke(
        new functions.auth.SignInRequest({
          phoneNumber: phone,
      ...
      result = await this.invoke(
        new functions.auth.CheckPasswordRequest({
          password: computeCheck(pwd, args.password),
        })
      );
    } else if (args.botToken) {
      result = await this.invoke(
        new functions.auth.ImportBotAuthorizationRequest({
          flags: 0,
  ...
  async sendCodeRequest(phone, forceSMS = false) {
    ...
    if (!phoneHash) {
      try {
        result = await this.invoke(
          new functions.auth.SendCodeRequest({
            phoneNumber: phone,
            apiId: this.apiId,
            apiHash: this.apiHash,
    ...
    if (forceSMS) {
      result = await this.invoke(
        new functions.auth.ResendCodeRequest({
          phone: phone,
          phoneHash: phoneHash,
        })
      );
  ...
  _handleUpdate(update) {
    ...
    if (
      update instanceof types.Updates ||
      update instanceof types.UpdatesCombined
    ) {
  ...
  async _getEntityFromString(string) {
    ...
    if (phone) {
      try {
        for (const user of (
          await this.invoke(new functions.contacts.GetContactsRequest(0))
        ).users) {
          if (user.phone === phone) {
            return user;
          }
      ...
      if (isJoinChat) {
        const invite = await this.invoke(
          new functions.messages.CheckChatInviteRequest({
            hash: username,
          })
        );
  ...
  async getEntity(entity) {
    ...
    if (
      entity instanceof types.InputPeerUser ||
      entity instanceof types.InputPeerSelf
    ) {
      const user = await this.invoke(
        new functions.users.GetUsersRequest({ id: [entity] })
      );
      return user[0];
    } else if (entity instanceof types.InputPeerChat) {
      const chat = await this.invoke(
        new functions.messages.GetChatsRequest({ id: [entity] })
      );
      return chat[0];
    } else if (entity instanceof types.InputPeerChannel) {
      const channel = await this.invoke(
        new functions.channels.GetChannelsRequest({ id: [entity] })
      );
      return channel[0];
    }
  }
  ...
  async getInputEntity(peer) {
    ...
    if (peer instanceof types.PeerUser) {
      const users = await this.invoke(
        new functions.users.GetUsersRequest({
          id: [
            new types.InputUser({
              userId: peer.userId,
              accessHash: 0,
      ...
      try {
        const channels = await this.invoke(
          new functions.channels.GetChannelsRequest({
            id: [
    ...
    throw new Error(
      `Could not find the input entity for ${
        peer.id || peer.channelId || peer.chatId || peer.userId
      }.
         Please read https://` +
  ...
  async _createExportedSender(dcId) {
    ...
    const auth = await this.invoke(
      new functions.auth.ExportAuthorizationRequest({ dcId: dcId })
    );
    const req = this._initWith(
      new functions.auth.ImportAuthorizationRequest({
        id: auth.id,
        bytes: auth.bytes,
      })
    );
      ...
      while (true) {
        ...
        try {
          result = await sender.send(
            new functions.upload.GetFileRequest({
  ...
  async downloadMedia(
    ...
    if (
      media instanceof types.MessageMediaPhoto ||
      media instanceof types.Photo
    ) {
      ...
      media instanceof types.MessageMediaDocument ||
      media instanceof types.Document
    ) {
      return await this._downloadDocument(
        media,
        file,
        date,
        args.thumb,
      ...
      media instanceof types.MessageMediaContact &&
      args.thumb == null
    ) {
      ...
      return await this._downloadWebDocument(
        media,
        file,
        args.progressCallback
      );
  ...
  async downloadProfilePhoto(entity, file, downloadBig = false) {
    ...
    let loc;
    if (
      photo instanceof types.UserProfilePhoto ||
      photo instanceof types.ChatPhoto
    ) {
      dcId = photo.dcId;
    ...
    try {
      ...
      if (e instanceof errors.LocationInvalidError) {
        ...
        if (ie instanceof types.InputPeerChannel) {
          const full = await this.invoke(
            new functions.channels.GetFullChannelRequest({
              channel: ie,
            })
          );
          return await this._downloadPhoto(
            full.fullChat.chatPhoto,
            file,
            null,
            null,
            thumb
  ...
  _getThumb(thumbs, thumb) {
    if (thumb === null || thumb === undefined) {
      ...
      thumb instanceof types.PhotoSize ||
  ...
  async _downloadPhoto(photo, file, date, thumb, progressCallback) {
    ...
    file = file ? file : Buffer;
    if (
      size instanceof types.PhotoCachedSize ||
      size instanceof types.PhotoStrippedSize
    ) {
