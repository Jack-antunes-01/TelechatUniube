...
class MTProtoState {
  ...
  _calcKey(authKey, msgKey, client) {
    ...
    const sha256a = Helpers.sha256(
      Buffer.concat([msgKey, authKey.slice(x, x + 36)])
    );
    const sha256b = Helpers.sha256(
  ...
  encryptMessageData(data) {
    ...
    const msgKeyLarge = Helpers.sha256(
      Buffer.concat([this.authKey.key.slice(88, 88 + 32), data, padding])
    );
    ...
    return Buffer.concat([
      keyId,
      msgKey,
      AES.encryptIge(Buffer.concat([data, padding]), key, iv),
    ]);
  }
  ...
  async decryptMessageData(body) {
    ...
    const ourKey = Helpers.sha256(
      Buffer.concat([this.authKey.key.slice(96, 96 + 32), body])
    );
