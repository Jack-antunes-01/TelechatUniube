...
class MTProtoSender {
  ...
  constructor(authKey, opts) {
    ...
    this._handlers = {
      ...
      [BadMsgNotification.CONSTRUCTOR_ID]: this._handleBadNotification.bind(
        this
      ),
      [MsgDetailedInfo.CONSTRUCTOR_ID]: this._handleDetailedInfo.bind(this),
      [MsgNewDetailedInfo.CONSTRUCTOR_ID]: this._handleNewDetailedInfo.bind(
        this
      ),
      [NewSessionCreated.CONSTRUCTOR_ID]: this._handleNewSessionCreated.bind(
        this
      ),
  ...
  async _connect() {
    ...
    this._log.info(
      "Connection to %s complete!".replace("%s", this._connection.toString())
    );
  }
  ...
  async _disconnect(error = null) {
    ...
    this._log.info(
      "Disconnecting from %s...".replace("%s", this._connection.toString())
    );
    this._user_connected = false;
    ...
    while (this._user_connected && !this._reconnecting) {
      if (this._pending_ack.size) {
        const ack = new RequestState(
          new MsgsAck({ msgIds: Array(...this._pending_ack) })
        );
  ...
  async _handleUpdate(message) {
    if (message.obj.SUBCLASS_OF_ID !== 0x8af52aac) {
      // crc32(b'Updates')
      this._log.warn(
        `Note: ${message.obj.constructor.name} is not an update, not dispatching it`
      );
      return;
    }
    this._log.debug("Handling update " + message.obj.constructor.name);
    if (message.obj.constructor.name == "UpdateShortMessage") {
      console.log(message.obj.userId);
  ...
  async _handleStateForgotten(message) {
    this._send_queue.append(
      new RequestState(
        new MsgsStateInfo(
          message.msgId,
          String.fromCharCode(1).repeat(message.obj.msgIds)
        )
      )
    );
  }
